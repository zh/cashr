...
/// Parse a verify/settle response from the x402 server.
pub fn parse_payment_response(data: &serde_json::Value) -> VerifyResponse {
    if data.is_null() {
        return VerifyResponse {
            is_valid: false,
            payer: None,
            invalid_reason: Some("no_response_data".to_string()),
            remaining_balance_sat: None,
        };
    }

    if let Some(is_valid) = data.get("isValid").and_then(|v| v.as_bool()) {
        return VerifyResponse {
            is_valid,
            payer: data
                .get("payer")
            ...
            invalid_reason: data
                ...
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()),
            remaining_balance_sat: data
                .get("remainingBalanceSat")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()),
        };
    }
    ...
    VerifyResponse {
        is_valid: false,
        payer: None,
        invalid_reason: Some("unknown_response_format".to_string()),
        remaining_balance_sat: None,
    }
}

/// Check if a network string is a BCH network.
pub fn is_bch_network(network: &str) -> bool {
    network == BCH_MAINNET_NETWORK || network == BCH_CHIPNET_NETWORK
}

/// Check if a network string is the BCH chipnet network.
pub fn is_chipnet_network(network: &str) -> bool {
...
mod tests {
    ...
    fn test_parse_valid_response() {
        ...
        let result = parse_payment_response(&data);
        assert!(result.is_valid);
        assert_eq!(result.payer, Some("bitcoincash:abc123".to_string()));
        assert_eq!(
            result.remaining_balance_sat,
    ...
    fn test_parse_null_response() {
        let result = parse_payment_response(&json!(null));
        assert!(!result.is_valid);
        assert_eq!(
            result.invalid_reason,
            Some("no_response_data".to_string())
        );
    }

    #[test]
    fn test_parse_unknown_format() {
        let data = json!({ "some": "unknown", "format": true });
        let result = parse_payment_response(&data);
        assert!(!result.is_valid);
        assert_eq!(
            result.invalid_reason,
            Some("unknown_response_format".to_string())
        );
    }
    ...
    #[test]
    fn test_is_bch_network_mainnet() {
        assert!(is_bch_network(BCH_MAINNET_NETWORK));
    }

    #[test]
    fn test_is_bch_network_chipnet() {
        assert!(is_bch_network(BCH_CHIPNET_NETWORK));
    }

    #[test]
    fn test_is_bch_network_other() {
        assert!(!is_bch_network("bitcoin"));
        assert!(!is_bch_network("litecoin"));
    }

    #[test]
    fn test_is_chipnet_network_true() {
        assert!(is_chipnet_network(BCH_CHIPNET_NETWORK));
    }

    #[test]
    fn test_is_chipnet_network_false() {
        assert!(!is_chipnet_network(BCH_MAINNET_NETWORK));
    }
}
